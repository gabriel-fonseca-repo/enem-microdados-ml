...
SELECT
  ...
  md.nota_matematica,
  md.nota_redacao,
  qs.Q02 AS q_formacao_pai,
  qs.Q03 AS q_formacao_mae,
  qs.Q05 AS q_renda_familia
FROM
  `basedosdados.br_inep_enem.microdados` AS md
INNER JOIN
  `basedosdados.br_inep_enem.questionario_socioeconomico_2010` AS qs
ON
  md.id_inscricao = qs.id_inscricao
WHERE 1=1
  AND qs.Q02 IS NOT NULL
  AND qs.Q03 IS NOT NULL
  AND qs.Q05 IS NOT NULL
